...
const taskController = container.resolve(TasksContoller);

tasksRoute.use((req, res, next) =>
  usersMiddleware.ensureTokenExists(req, res, next),
);
tasksRoute.post(
  '',
...
tasksRoute.get('', taskController.findAll.bind(taskController));
tasksRoute.get(
  '/:taskId',
...
tasksRoute.post(
  ...
  (req, res, next) => tasksMiddleware.ensureTasksIdExists(req, res, next),
  (req, res, next) =>
    tasksMiddleware.ensureTaskDontHaveAnDeadLine(req, res, next),
...
tasksRoute.patch(
  ...
  (req, res, next) => tasksMiddleware.ensureTasksIdExists(req, res, next),
  (req, res, next) =>
    tasksMiddleware.ensureTasksDeadlineIdExists(req, res, next),
