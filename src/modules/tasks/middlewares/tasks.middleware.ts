...
class TasksMiddleware {
  ...
  async ensureTaskDontHaveAnDeadLine(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
    ...
    if (task!.taskDeadline)
      return res.status(409).json({ message: 'Task already have a Deadline!' });

    return next();
  }

  async ensureTasksDeadlineIdExists(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
    ...
    const deadline: TaskDeadline | null =
      await this.deadlineRepository.findOneBy({
        id: deadlineId,
      });

    if (!deadline)
      return res.status(404).json({ message: 'Task Deadline not Found!' });
